...
export class Kernel {
  ...
  private choose(obs: Float32Array): number {
    ...
    imagineScores({
      ...
      // Horizon 2: treat the predicted window as a new observation, score every
      // follow-up with pure model (no true whiskers). Confidence-gated inside.
      predictFrom: (pred, a1, into) => this.loop.imagine(this.encode(pred, a1), into),
      readPred: (pred, a) => {
        const tx = R + DIRS[a].x;
        const ty = R + DIRS[a].y;
        const ti = (ty * VIEW + tx) * CH;
        // Pure model: destination cell lives inside the first-step prediction.
