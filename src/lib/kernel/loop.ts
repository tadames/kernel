...
export class Loop {
  ...
  /**
   * Visit-scent novelty gate for imagination only. Plan surprise, mute,
   * and ρ stay side-channel-zero via familyWeight(2) === 0.
   *
   * After the 96-step probe, a family-2 residual that has not fallen
   * stays shut. When it has fallen (or the probe is still open), the
   * destination cell scales by its own residual EMA: compressible
   * local flow can pull, noise cannot.
   */
  scentNovelty(cell: number): number {
    if (this.familyCount !== 3 || this.scentCells <= 0) return 0;
    const fallen = this.familyResidual[2] < 0.05;
    if (this.scentTrains >= 96 && !fallen) return 0;
    const c = Math.max(0, Math.min(this.scentCells - 1, cell | 0));
    const src = this.residualEma[c * 3 + 2];
    return 1 / (1 + src * 14);
  }
